...
class SystemService extends Service {
  ...
  async saveSystemData(ctx) {
    ...
    // 检验系统是否存在
    const search = await ctx.model.System.findOne({
  ...
  async updateSystemData(ctx) {
    ...
    const result = await this.ctx.model.System.updateOne(
      { app_id: appId },
      update,
      { multi: true }
    ).exec();
  ...
  async getSystemForDb(appId) {
    ...
    return (
      (await this.ctx.model.System.findOne({ app_id: appId }).exec()) || {}
    );
  }
  ...
  async getSysForUserId(ctx) {
    ...
    if (!token) return [];
    return (
      (await ctx.model.System.where('user_id')
        .elemMatch({ $eq: token })
        .exec()) || []
    );
  }
   ...
   */
    ...
    return await this.ctx.model.System.updateOne(
      { app_id: appId },
  ...
  async deleteSystem(appId) {
    const result = await this.ctx.model.System.findOneAndRemove({
      app_id: appId,
    }).exec();
