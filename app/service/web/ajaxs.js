...
class AjaxsService extends Service {
  // 获得页面性能数据平均值
  async getPageAjaxsAvg(appId, url, beginTime, endTime, markPage) {
    const query = {
      $match: { call_url: url, speed_type: 1, mark_page: markPage },
    };
    if (beginTime && endTime) {
      query.$match.create_time = {
  ...
  async getAverageAjaxList(ctx) {
    ...
    return url
      ? await this.oneThread(appId, queryjson, pageNo, pageSize, group_id)
  ...
  async getOneAjaxList(appId, url, pageNo, pageSize, beginTime, endTime, type) {
    ...
    const count = Promise.resolve(
      this.app.models.WebAjaxs(appId).count(query.$match).exec()
    );
    const datas = Promise.resolve(
      this.app.models
        .WebAjaxs(appId)
