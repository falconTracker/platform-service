...
class ResourceService extends Service {
  // 单页页面性能数据列表（简单版本）
  async getResourceForType(appId, url, speedType, pageNo, pageSize, markPage) {
    pageNo = pageNo * 1;
    pageSize = pageSize * 1;
    speedType = speedType * 1;

    const query = { $match: { url, speed_type: speedType, mark_page: markPage } };
