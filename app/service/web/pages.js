...
class PagesService extends Service {
  ...
  async getOnePageList(ctx) {
    ...
    const count = Promise.resolve(
      this.app.models.WebPages(appId).count(queryjson.$match).read('sp')
        .exec()
    );
    const datas = Promise.resolve(
      this.app.models
  ...
  async getPageDetails(appId, id) {
    let page = await this.app.models
      .WebPages(appId)
      .findOne({ _id: id })
    // .read("sp")
      .exec();
    ...
    const mark_page = page.mark_page;
    const metric = await this.app.models
      .WebMetrics(appId)
      .find({ mark_page })
      .exec();
    return {
