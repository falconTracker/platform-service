...
class utilfn {
    ...
    if (json.code) {
      ...
      switch (XMLHttpRequest.status) {
        ...
        case 502:
          popup.alert({
            type: "msg",
            title: XMLHttpRequest?.responseJSON?.message || "服务器内部错误",
          });
          break;
        default:
  ...
  getSearchTime() {
    ...
    let endTime = current.getTime();

    // 将当前时间设置为当天的 0 点
    let begin = new Date(endTime);
    begin.setHours(0, 0, 0, 0);
    let beginTime = begin.getTime();

    return {
      beginTime: new Date(beginTime).format("yyyy/MM/dd hh:mm:ss"), // 当天 0 点
