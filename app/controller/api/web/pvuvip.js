...
class PvUvIpController extends Controller {
  ...
  async getPvUvIpSurvey() {
    ...
    const timeStart = new Date(interval.prev().toString()).getTime();
    const betweenTime = Math.abs(new Date(timer).getTime() - timeStart);
  ...
  async getPvUvIpList() {
    ...
    const beginTime =
      query.beginTime || new Date(now.getTime() - 1000 * 60 * 60);
    const datalist =
      ...
      try {
        ...
        datalist.forEach(item => {
          if (date.getTime() === new Date(item.create_time).getTime()) {
            ...
            items.flow = Number(((item.flow || 0) / 1024 / 1024).toFixed(2));
          }
        });
        result.push(items);
      } catch (e) {
